...
def MMSE_STSA(args):
    ...
    magnitude_noisy_test, phase_noisy_test = divide_magphase(stft_noisy_test, power=1)   # A(x) = |A(x)|e^{jθ}
    ...
    for i in range(nFFT2):                         
        ...
        #operator [2](52)
        oper=aPosterioriSNR_frame-1     # γ(k,l)-β_γ，β_γ常设置为1
        oper[oper < 0] = 0              # max{γ(k,l)-β_γ, 0}
        ...
        gain= smoothed_a_priori_SNR/(1+smoothed_a_priori_SNR)  # α_g=2, β_g=1
        if any(V<1):
        ...
        previousGainedaPosSNR = (gain**2) * aPosterioriSNR_frame  # G^2(k,l-1)γ(k,l-1)
    ...
    signal_reconstructed_clean =librosa.istft(np.array(stft_reconstructed_clean), hop_length=hop_length_sample, window=winfunc)
