...
class QualityNetAttn(nn.Module):
    ...
    def __init__(self, dropout=0.3) -> None:
        ...
        self.lstm = nn.LSTM(257, 100, num_layers=1, bidirectional=True, dropout=dropout, batch_first=True)
        self.linear1 = nn.Linear(200, 50)  # 2 * 100
        self.elu = nn.ELU()
        self.dropout = nn.Dropout(dropout)
...
def floatToOnehot(x, step):
    x = x - 1.0
    x = x // step
    x = torch.nn.functional.one_hot(x, int(4 / step))
    return x
