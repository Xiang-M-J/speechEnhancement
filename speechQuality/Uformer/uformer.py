...
class Uformer(nn.Module):
    ...
    def forward(self, inputs, src):
        ...
        inputs_com = torch.stft(inputs[:,0], n_fft=self.fft_len, hop_length=self.win_inc, win_length=self.win_len, window=torch.hann_window(self.win_len).cuda(), return_complex=True)# B F T 2
        inputs_real = torch.real(inputs_com).unsqueeze(1)
        inputs_imag = torch.imag(inputs_com).unsqueeze(1)

        src_com = torch.stft(src[:,0], n_fft=self.fft_len, hop_length=self.win_inc, win_length=self.win_len, window=torch.hann_window(self.win_len).cuda(), return_complex=True)  # B F T 2
        src_real = torch.real(src_com).unsqueeze(1)
        src_imag = torch.imag(src_com).unsqueeze(1)
        ...
        spk_com = torch.complex(real, imag)
        spk1 = torch.istft(spk_com, n_fft=self.fft_len, hop_length=self.win_inc, win_length=self.win_len, window=torch.hann_window(self.win_len).cuda(), center=True)
        output.append(spk1)
