...
def train(model, train_loader, valid_loader, args):
    ...
    with open(logger_path, 'w') as logger_file:
        logger_file.write('epoch,train_loss,HASQI,valid_HASQI,Pearson_cc\n')
    ...
    for epoch in range(1, args.train_epoch + 1):
        ...
        epoch_valid_loss, epoch_valid_hasqi, epoch_valid_hasqi_fram, epoch_valid_hasqi_avg, Pearson_cc_hasqi = evaluate(
            model, valid_loader, epoch, args)

        print(f'Epoch:{epoch}')
        print(f'Train loss:{epoch_train_loss:.5f},HASQI:{epoch_hasqi:.5f},{epoch_hasqi_fram:.5f},{epoch_hasqi_avg:.5f}')

        print(
            f'Validation loss:{epoch_valid_loss},HASQI:{epoch_valid_hasqi:.5f},{epoch_valid_hasqi_fram:.5f},{epoch_valid_hasqi_avg:.5f},Pearson_cc:{Pearson_cc_hasqi}')

        with open(logger_path, 'a') as logger_file:
            logger_file.write(
                f'{epoch:03d},{epoch_train_loss:4.6e},{epoch_hasqi:1.6e},{epoch_valid_loss:4.6e},{epoch_valid_hasqi:1.6e},[{Pearson_cc_hasqi:3.6e}]\n')
    ...
    epoch_train_loss, epoch_hasqi = None, None
...
def evaluate(model, valid_loader, epoch, args):
    ...
    epoch_valid_loss, epoch_valid_hasqi = None, None
    epoch_valid_hasqi_fram, epoch_valid_hasqi_avg = None, None
    ...
    print('data,TrueHASQI,PredictHASQI', file=output_file)

    with torch.no_grad():
        valid_loss, valid_hasqi = 0, 0
        valid_hasqi_fram, valid_hasqi_avg = 0, 0
        for step, (name, Sxx_ref, Sxx_data, hl, HASQIscore, hltype) in enumerate(valid_loader):
            Sxx_data = Sxx_data.cuda(non_blocking=True)
            ...
            for i, j, k in zip(name, hasqi_score, predict_hasqi_score):
                print(i, j, k, sep=',', file=output_file)
        ...
        hasqi, predict_hasqi = df['TrueHASQI'].astype('float32').to_numpy(), df['PredictHASQI'].astype(
            'float32').to_numpy()
    ...
    return epoch_valid_loss, epoch_valid_hasqi, epoch_valid_hasqi_fram, epoch_valid_hasqi_avg, Pearson_cc1
...
def test(model, test_loader, mode, args):
    ...
    test_total_loss, test_total_hasqi = None, None
    total_steps = int(len(test_loader))
    print(len(test_loader))

    with torch.no_grad():
        test_loss, test_hasqi = 0, 0
        for step, (name, Sxx_ref, Sxx_data, hl, HASQIscore) in enumerate(test_loader):
            Sxx_data = Sxx_data.cuda(non_blocking=True)
            hl = hl.cuda(non_blocking=True)
            HASQIscore = HASQIscore.cuda(non_blocking=True)
            ...
            for i, j, k, l, m, n, o in zip(name, hasqi_score, predict_hasqi_score, haspi_score, predict_haspi_score,
                                           hltype, hl.cpu().numpy()):
