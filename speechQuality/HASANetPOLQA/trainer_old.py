...
def train(model, train_loader, valid_loader, args):       
    ...
    for epoch in range(1,args.train_epoch+1):
        ...
        epoch_valid_loss, epoch_valid_hasqi, epoch_valid_hasqi_fram, epoch_valid_hasqi_avg, Pearson_cc_hasqi = evaluate(model, valid_loader, epoch, args)
        
        print(f'Epoch:{epoch}')
        print(f'Train loss:{epoch_train_loss:.5f},HASQI:{epoch_hasqi:.5f},{epoch_hasqi_fram:.5f},{epoch_hasqi_avg:.5f}')
        
        print(f'Validation loss:{epoch_valid_loss},HASQI:{epoch_valid_hasqi:.5f},{epoch_valid_hasqi_fram:.5f},{epoch_valid_hasqi_avg:.5f},Pearson_cc:{Pearson_cc_hasqi}')
        
        with open(logger_path, 'a') as logger_file:
            logger_file.write(f'{epoch:03d},{epoch_train_loss:4.6e},{epoch_hasqi:1.6e},{epoch_valid_loss:4.6e},{epoch_valid_hasqi:1.6e},[{Pearson_cc_hasqi:3.6e}]\n')
